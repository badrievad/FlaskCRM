document
	.getElementById('merge-deals-button')
...
async function getSelectedDeals() {
	...
	for (let checkbox of document.querySelectorAll('.deal-checkbox:checked')) {
		...
		var company = checkbox
			.closest('tr')
			.querySelector('.deal-title')
			.textContent.trim()
		var manager = checkbox
			.closest('tr')
			.querySelector('.deal-manager')
			.textContent.trim()
...
function canMergeDeals(deals) {
	...
	const canMerge = deals.every(
		deal =>
			deal.company === firstDealCompany && deal.manager === firstDealManager
	)
...
async function checkSelectedDeals() {
	...
	document.getElementById('merge-deals-button').disabled =
		!canMergeDeals(selectedDeals)
}
		...
		.then(data => {
			...
			data.deals.forEach((deal, index) => {
				...
				if (
					...
					currentUser.fullname === deal.created_by
				) {
					checkboxCell = `
				...
				const dealItem = `
					...
					deal.id
                        ...
                        <td class="deal-manager">
                            <img src="${dealUserIcon}" alt="User Photo" class="profile-image-deal"> ${
					deal.created_by
				}
                        </td>
                        <td class="deal-created">${timeAgo}</td>
                        <td>
                            <div class="btn-container">
                                <button class="icon-button" onclick="event.stopPropagation(); confirmArchive(${
																	deal.id
																})">
...
function formatTimeAgo(dateString) {
	...
	if (diffInSeconds < 60) {
		...
		timeAgo = `${minutes} ${getPlural(minutes, [
			'минута',
			'минуты',
			'минут',
		])} назад`
